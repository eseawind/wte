...
function OrgShow(OrgNode_){
	...
	var opt='top=0, left=0, toolbar=no, menubar=no, scrollbars=yes, resizable=yes,location=no, status=no';   

	function GetBoxTemplet(){//取得节点模板
		if(This.BoxTemplet!=null)return This.BoxTemplet;
		var TempletStr="<div  id=\"{Id}\" style=\"font-size:12pt;padding:5px 5px 5px 5px;width:150px;;border:0px; clear:left;float:left;text-align:center;position:absolute;"
		if(This.ShowType==2)TempletStr+="writing-mode: tb-rl;";
		TempletStr+="\" title=\"{Description}\" ><a onclick=\"{Link}\"  target=\"_blank\">{Text}</a></div>";
		return TempletStr;
	}
