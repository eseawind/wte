	...
	<#list knowledgeNodes as knowledge>
		<tr>
	    	<td width=700px height=30px >
				${flag} 
	    		<span><@i18n "title_name" />: </span><input value="${knowledge.name?if_exists}" style="width: 220px; height: 20px;" onchange="javascript:action_knowledge('update.do','column=name&id=${knowledge.id?if_exists}&columnValue='+this.value)" /> 
			    <!--
			    <span><@i18n "title_alias" />: </span><input value="${knowledge.alias?if_exists}" style="width: 220px; height: 20px;"  onchange="javascript:action_knowledge('update.do','column=alias&id=${knowledge.id?if_exists}&columnValue='+this.value)" /> 
			-->
			</td>
