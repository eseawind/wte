					...
					<div class="panel-body">
					 <table class="table table-condensed table-bordered table-striped">
						...
						<tr>
							...
							<td><input type="text" disabled  value="<#if rhs["paper"]?exists >${rhs["paper"].totalmark?if_exists}</#if>" onKeyUp="this.value=this.value.replace(/[^\.\d]/g,'');if(this.value.split('.').length>2){this.value=this.value.split('.')[0]+'.'+this.value.split('.')[1]}"/></td>
