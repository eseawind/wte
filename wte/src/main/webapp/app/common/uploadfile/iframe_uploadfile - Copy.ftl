...
<span id="div_new_filelist" style="display:none">${rhs["info"]?if_exists}
	<#list rhs["uploadFiles"] as x>
		<div id='${x.id}'><a href='${rhs.contextPath + x.url}'>${x.fileName}</a>
		<#if rhs["readonly"]!="readonly" >
		<a href='#' class="btn btn-info btn-xs" onclick="javascript:del_file('common_uploadfile_delete_uploadfile.do','${x.id}')" ><i class='icon-remove'></i>&nbsp;Delete</a></div>
		</#if>
	</#list>
</span>
